...
integer :: localped(2,31),pedbreed(2,31)               !For storing pedigree of each individual
...
integer,allocatable :: genpedpoint(:)                   !For pointing to the 

character*1,allocatable :: cbhapl(:,:)                      ! For keeping the haplotype of the crossbred animal working with each time
!character*1,allocatable :: pedhapl(:,:,:,:)                 !Matrix for keeping the haplotypes from local pedigree
...
integer,allocatable :: breedo(:,:),breedo2(:,:)
...
!logical,allocatable :: matchf(:),matchm(:),matchg(:)
...
do i=1,nocbs
  read(10,*)breed(i+l),ndsbr(1,i),(nubrcr(1,i,j),j=1,ndsbr(1,i))
  read(10,*)breedc,ndsbr(2,i),(nubrcr(2,i,j),j=1,ndsbr(2,i))
  if(breedc.ne.breed(i+l))then
...
allocate(pbhapl(nopb,2,nosnp))             !Allocating a matrix for the purebred haplotypes
...
do i=1,nopb
  read(11,'(a)')readline
  read(readline,*)pbid(i),breedc
  do j=1,nopbs                                            !finding the matchin purebred code
    l=j
    if(breedc.eq.breed(j))exit
    if(j.eq.nopbs)write(20,*)'undefined purebreed: ',breedc
  enddo
  if(hapread)then
    read(12,*)pbhapl(pbanp(l),1,:)
    read(12,*)pbhapl(pbanp(l),2,:)
  else
    read(12,hapform)pbhapl(pbanp(l),1,:)
    read(12,hapform)pbhapl(pbanp(l),2,:)
  endif
  pbanbreed(pbanp(l))=l
  pbanp(l)=pbanp(l)+1
  if(pbanp(l).gt.pbpoint(l,2)+1)then
...
allocate(ped(3,noped),genpedpoint(noped),numped(2,noped))
allocate(pedbrcode(noped))
genpedpoint=0
numped=0
rewind(11)
...
allocate(cbhapl(2,nosnp))      !For keeping haplotypes of the current
...
allocate(allcom(nopb,2,norounds))
...
call crossposss(nopbs2,crossposs)


	  !Possible breeds and finding purebred with genotype in the pedigree
!	  crosslog=.FALSE.
...
!Format for the breedoforigin2 output needs to be longer for more than 4 breeds
if(nopbs.lt.5)then
...
!run throug crossbred boacode file
!looking for errors
...
do i=1, nocb
  ...
  if(hapread)then
    read(12,*,iostat=ierr)cbhapl(1,:)
    read(12,*,iostat=ierr)cbhapl(2,:)
  else
    read(12,hapform,iostat=ierr)cbhapl(1,:)
    read(12,hapform,iostat=ierr)cbhapl(2,:)
  endif
  if(ierr.ne.0.and.i.lt.nocb)then
  ...
  call compall(cbhapl,pbhapl,roundvis,allcom,nosnp,nopb,norounds,nummm)
  
!Input: hapl of CB, hapl of all PB,roundvis,output,num of snp,num of PB, number of rounds,num of mismatces
...
!      write(*,*)breedlog
  ...
  ! NEW (7.3, june 2022) stuff starts here
  !First, instead of putting pedigree haplotypes to an array, I update the localped array from having numped to having
  ! the position of the PB haplotype, and 0 if not genotyped. 
  if(useped)then
    do j=1,nulocped
      if(genpedpoint(localped(1,j)).gt.0)then
	    pedbreed(1,j)=pbanbreed(genpedpoint(localped(1,j)))
		localped(1,j)=genpedpoint(localped(1,j))
	    pedhaplog(1,j)=.TRUE.
	  else
	    localped(1,j)=0
      endif
      if(genpedpoint(localped(2,j)).gt.0)then
	    pedbreed(2,j)=pbanbreed(genpedpoint(localped(2,j)))
        localped(2,j)=genpedpoint(localped(2,j))
	    pedhaplog(2,j)=.TRUE.
	  else
	    localped(2,j)=0
      endif
  ...
  if(dbrnow(2).eq.0.and.sbrnow(2).eq.0)then
    do j=1,nosnp
      ...
      elseif(breedo(2,j).eq.9.and.breedo(1,j).eq.dbrnow(1))then
        breedo(2,j)=sbrnow(1)
      elseif(cbhapl(1,j).eq.cbhapl(2,j))then
        res(1)=sbrnow(1)
        res(2)=dbrnow(1)
  ...
  elseif(sbrnow(2).eq.0)then      !if only one sire breed
    do j=1,nosnp
      ...
      elseif(cbhapl(1,j).eq.cbhapl(2,j).and.breedo(1,j).gt.8.and.breedo(2,j).gt.8)then
        call brass1(sbrnow(1),breedo(:,j),logass(:,:,:,j),nopbs,1)
      endif
    enddo
  ...
  write(23,propformat)cbid(i),propanim(1,i),propanim(2,i),(asscount(1,k),k=1,nopbs),asscount(1,9),(asscount(2,l),l=1,nopbs),&
  asscount(2,9)
...
subroutine getapix1(a,b)         !a is 1 for pateranal 2 for maternal, b is for allele 1 and 2
integer :: a,b
  do l=1,nopbs                                                          !loop over all breeds
    ...
    if(useped)then
      do j=1,nulocped                                                     !loop over the genotypes that were found in the pedigree
        if(pedhaplog(a,j).and.pedbreed(a,j).eq.l)then
          do n=1,norounds
            if(.not.matchf(n).and.allcom(localped(a,j),b,n))then
			  apix1(b,l,n)=prod(j)           !allcom(nopb,2,norounds)
			  matchf(n)=.TRUE.
			endif
	      enddo
	    endif
    ...
    if(all(matchf))cycle
    if(usepop)then
	  comout=count(allcom(pbpoint(l,1):pbpoint(l,2),b,:),dim=1)
	  do n=1,norounds
	    if(matchf(n))cycle
...
subroutine compall(anhap,pbset,rc,mats,n,m,l,nmm)    
 !Input: hapl of CB, hapl of all PB,roundvis,output,num of snp,num of PB, number of rounds,num of mismatces
  implicit none
  integer :: n,m,l
  character*1,intent(in) :: anhap(2,n),pbset(m,2,n)
  integer :: i,j,k,o,c
  integer :: nmm(l),rc(3,l)
  logical,intent(out) :: mats(m,2,l)
  logical :: chap(n)
  mats=.FALSE.
  do i=1,m           !across all pb
    do j=1,2         !two haplotypes of pb
	  do k=1,2         !two haplotypes of cb
        chap(:)=anhap(k,:).ne.pbset(i,j,:)
		do o=1,l   !across rounds
		  c=count(chap(rc(1,o):rc(2,o)))
		  mats(i,k,o)=mats(i,k,o).or.(c.le.nmm(o))
		enddo
      enddo
    enddo
  enddo
...
subroutine gapfi2(snp,intbr,logbr,wl,br)  !Filling in based on neighbouring, both strands
  implicit none
...
subroutine gapfi2e(snp,intbr,logbr,wl,br)  !Filling in based on neighbouring, both strands, at the ends of chromosome
  implicit none
  integer,intent(in) :: snp,wl,br
  integer :: intbr(2,snp)
  integer :: a,b,j
...
subroutine gapfi1(snp,intbr,logbr,wl,br,c)         !neighbouring filling, one strand
  implicit none
...
subroutine gapfi1e(snp,intbr,logbr,wl,br,c)  !Filling in based on neighbouring, both strands, at the ends of chromosome
  implicit none
  integer,intent(in) :: snp,wl,br,c
  integer :: intbr(2,snp)
  integer :: a,b,j
