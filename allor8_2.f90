...
!Fixed a bug and added to output in July 2024 (8.2)
...
write(20,*)'=============================================================='
write(20,*)'        '
write(20,*)'                  AllOr version 8.2'
write(20,*)'        '
write(20,*)'=============================================================='
write(*,*)'Starting AllOr 8.2'
...
if(nopbs.gt.8)then
  write(20,*)nopbs,'are too many breeds, the maximum is 8 '
...
do i=1,nopbs
  ...
  if(addpure(i))write(20,*)'Haplotypes from crossbred will be added to haplotype library'
enddo
...
do i=1,nocbs
  ...
  if(breedc.ne.breed(i+nopbs))then
    write(*,*)'breed defination error: ',breed(i),breedc
...
read(10,*)noped
read(10,*)peddepth
if(noped.eq.0.or.peddepth.eq.0)then
  write(20,*)'No pedigree will be used'
  useped=.FALSE.
endif
read(10,'(a)')pedfile
read(10,'(a)')misscode
...
if(i.ne.nopb)then
  write(*,*)'Total number of purebred reference in control.txt and ',boacodefile(1), ' do not match',i,nopb
  write(20,*)'Total number of purebred reference in control.txt and ',boacodefile(1), ' do not match',i,nopb
  write(*,*)'Program stops'
...
!-------------------------addition dec 23
  ...
  if(l.gt.nopbs)then
    write(20,*)'Undefined purebreed: ',readid,breedc
	write(20,*)'Will be skipped' 
	cycle
  endif
  if(hapread)then
    read(12,*)hapl(1,:)
...
write(20,*)'Number of overlapping windows: ',norounds

!Allocating for working with crossbred
...
call makeroundvis(nosnp, norounds, control, roundvis,p)
!number of SNP, number of rounds, the control vector, the roundvis output arra,p
!The roundvis array has the start (1) and end (2) points for each round and the number of SNP in the window (3)
...
write(20,*)'Start reading ',haplfile(2),' and assigning BOA'
...
do i=1, nocb
  ...
  do j=1,nocbs
    ...
    if(j.eq.(nocbs))then
	  ...
	  write(20,*)'If that was not the intention please check the codes in control.txt and ',boacodefile(2)
    endif
  enddo
  ...
  if(useped)then
    do j=1,noped
      ...
      if(j.eq.noped)then
        ...
        if(pedlog)then
          pedlog=.FALSE.
          write(20,*)'no breedcode - will allow all breeds for paternal and maternal alleles' 
...
write(20,*)'Proprtion assigned: ',propall
...
write(*,*)'AllOr finished'
write(*,*)'Please check allor.log for information on the progress'
close(20)
...
subroutine makeroundvis(a,b,c,d,e)
!number of SNP, number of rounds, the control vector, the roundvis output array, p
  implicit none
  integer a, b, e, f,g
  integer c(3)
  integer d(3,b)
